        ...
        for (let i = 0; i < tables.length; i++) {
            ...
            for (let j = 0; j < bodyRows.length; j++) {
                ...
                for (let t = 0; t < bodyCells.length; t++) {
                    // Get the cell label by counting which column this cell is in, counting that
                    // far in to the table head columns, and getting the innerHTML
                    let label = headCells[t].innerHTML;
...
function addMobileLabels(t, bodyCells, label) {
    // If the cell does not already contain a span...
...
setTimeout(function() {
    mobileTables();
    console.log('The mobileTables function has run on page load!');
}, 100)
